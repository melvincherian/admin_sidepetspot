...
Stream<List<petProductModel>>fetchProducts(){
  return _firestore.collection('products').snapshots().map((snapshot){
    return snapshot.docs.map((doc)=>petProductModel.fromJson(doc.data() as Map<String,dynamic>)).toList();
  });
}
...
Future<void>updateProduct(String productId, petProductModel updateProduct)async{
try{
  await _firestore.collection('products').doc(productId).update(updateProduct.toJson());
}catch(e){
